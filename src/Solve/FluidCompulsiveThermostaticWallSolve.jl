...
function eachStep!(
    ...
    termostatic_wall_particles::ThermostaticWallParticlesType where {ThermostaticWallParticlesType <: AbstractVector{<:ThermostaticWallParticle}},
    ...
    fluid_thermostatic_wall_neighbours_task = @async begin
        update!(
            ...
            [p.x_vec_ for p in fluid_particles],
            [p.x_vec_ for p in termostatic_wall_particles];
            cutoff = smooth_kernel.influence_radius_,
        )
        findNeighbours!(
    ...
    fluid_wall_force_task = @async begin
        @floop @simd for fluid_wall_neighbour in fluid_wall_neighbours
            ...
            @inbounds pressureForce!(fluid_particles[fluid_wall_neighbour.i_], fluid_wall_neighbour, smooth_kernel, th_wc_lm)
            @inbounds viscosityForce!(
    ...
    fluid_thermostatic_wall_force_task = @async begin
        @floop @simd for fluid_thermostatic_wall_neighbour in fluid_thermostatic_wall_neighbours
            @inbounds wallForce!(
                ...
                termostatic_wall_particles[fluid_thermostatic_wall_neighbour.j_],
...
function solve!(
    ...
    writeVTP(0, 0.0, vtp_io, [fluid_particles, wall_particles, termostatic_wall_particles])
    ...
    fluid_thermostatic_wall_neighbour_system = InPlaceNeighborList(
        x = [p.x_vec_ for p in fluid_particles],
        y = [p.x_vec_ for p in termostatic_wall_particles],
        cutoff = smooth_kernel.influence_radius_,
        parallel = true,
    )
    fluid_neighbours = CommonNeighbour[] # to be initialized
    fluid_wall_neighbours = CommonNeighbour[] # to be initialized
    fluid_thermostatic_wall_neighbours = CommonNeighbour[] # to be initialized
    for step in ProgressBar(1:(dr_fti_forward_euler.total_step_))
        ...
        #     writeVTP(step, step * dr_fti_forward_euler.dt_, vtp_io, [fluid_particles, wall_particles, termostatic_wall_particles])
        # end
        if isOutputStep(step, dr_fti_forward_euler)
            writeVTP(
                ...
                vtp_io,
                [fluid_particles, wall_particles, termostatic_wall_particles],
            )
        end
    end
    return nothing
