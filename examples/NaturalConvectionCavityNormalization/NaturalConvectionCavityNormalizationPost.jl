...
function getData(step::Int64)
    ...
    temperature = (vtp_file_component |> get_point_data)["Temperature"] |> get_data
    return points[1, :], points[2, :], temperature
end
...
function plot(step::Int64)
    ...
    ax1 = Axis(
        fig[1, 1],
        title = "Step=$(string(step))  Time=$(string(round(step*total_time/length(file_lists), digits=4)))  Temperature",
        aspect = DataAspect(),
    )
