...
function getData(step::Int64)
    ...
    velocitys = (vtp_file_component |> get_point_data)["Velocity"] |> get_data
    velocitys_norm = sqrt.(sum(velocitys .^ 2, dims = 1)) |> vec
    return points[1, :], points[2, :], points[3, :], velocitys_norm
end

# ╔═╡ 10f88558-54dd-4a82-9c99-39083bc22618
function plot(step::Int64, view::Int64)
    x, y, z, v = getData(step)
    fig = Figure(size = (600, 400))
    ax1 = Axis3(
        fig[1, 1],
        title = "Step=$(string(step))  Time=$(string(round(step*total_time/length(file_lists), digits=4)))  Velocity",
        aspect = (1, 1, 1),
        azimuth = view * pi / 20,
    )
...
# ╔═╡ 0a57ee49-c1d5-4082-947e-cc01b12f133e
...
manifest_format = "2.0"
project_hash = "f0068976ae6fc0b584bf51711c802b38fc576bab"
