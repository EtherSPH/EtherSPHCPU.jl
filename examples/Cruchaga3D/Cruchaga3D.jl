...
const dir_path = "./examples/Cruchaga3D/Cruchaga3DData";
...
water_particles = createBoxParticles(CommonLiquidParticle, dr, x0, y0, z0, water_a_number, water_b_number, water_c_number);
for index in eachindex(water_particles)
    water_particles[index].p_ = rho_0 * gravity * (water_b - water_particles[index].x_vec_[2])
    water_particles[index].rho_ = (water_particles[index].p_ / wc_lm.b_ + 1.0)^(1.0 / gamma) * rho_0
...
for index in eachindex(bottom_wall_particles)
    bottom_wall_particles[index].normal_vec_ = [0.0, 1.0, 0.0]
...
for index in eachindex(top_wall_particles)
    top_wall_particles[index].normal_vec_ = [0.0, -1.0, 0.0]
...
for index in eachindex(front_wall_particles)
    front_wall_particles[index].normal_vec_ = [0.0, 0.0, 1.0]
...
for index in eachindex(back_wall_particles)
    back_wall_particles[index].normal_vec_ = [0.0, 0.0, -1.0]
    back_wall_particles[index].gap_ = dr
end

left_wall_particles = createBoxParticles(CompulsiveWallParticle, dr, x0 - box_thick_number * dr, y0, z0, box_thick_number, box_b_number, box_c_number);
for index in eachindex(left_wall_particles)
    left_wall_particles[index].normal_vec_ = [1.0, 0.0, 0.0]
    left_wall_particles[index].gap_ = dr
end

right_wall_particles = createBoxParticles(CompulsiveWallParticle, dr, x0 + box_a_number * dr, y0, z0, box_thick_number, box_b_number, box_c_number);
for index in eachindex(right_wall_particles)
    right_wall_particles[index].normal_vec_ = [-1.0, 0.0, 0.0]
    right_wall_particles[index].gap_ = dr
end

wall_particles = vcat(bottom_wall_particles, top_wall_particles, front_wall_particles, back_wall_particles, left_wall_particles, right_wall_particles);

check(particle::AbstractParticle)::Bool = true;

main() = @fastmath solve!(water_particles, wall_particles, smooth_kernel, wc_lm, dr_fti_forward_euler, vtp_io; check = check);
